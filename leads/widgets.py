...
class SourceIdWidget(widgets.MultiWidget):
    ...
    widget_template = '<div class="form-row"><ul id="%s" class="keyvaluewidget">%s</ul></div>'
    ...
    def __init__(self, attrs=None, key_attrs=None, value_attrs=None):
        ...
        attrs = attrs or {}
        attrs.setdefault('class', 'TextField')
        self.attrs = attrs
        self.input_widget = self.input_widget_class(attrs)
        super(SourceIdWidget, self).__init__([], attrs)
    ...
    def render(self, name, value, attrs=None):
        ...
        if value:
            ...
            empty_row = ''.join([  # row for cloning in js
                self.make_input_widget('key', name, 'Source name', '', main_id, '', final_attrs),
                self.make_input_widget('value', name, 'Matching property ID', '', main_id, '', final_attrs),
            ...
            for i, (key, val) in enumerate(values, start=1):
                output.extend(self.row_template % ('', ''.join([
                    self.make_input_widget('key', name, 'Source name', key, main_id, i, final_attrs),
                    self.make_input_widget('value', name, 'Matching property ID', val, main_id, i, final_attrs),
                    self.make_del_link(name, main_id, i),
                ])))
            return mark_safe(self.format_output(name, main_id, output))
        return ''

    def make_input_widget(self, widget_type, name, label, value, main_id, index, attrs):
